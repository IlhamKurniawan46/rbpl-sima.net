...
import { getCustomerByProfileId } from '@/lib/mock-data';
      ...
      <div className="p-4 space-y-4">
        <div className="bg-white rounded-2xl p-5 shadow-[var(--shadow-card)] border border-border-light text-center">
          ...
          <div className="mt-4 space-y-2.5 text-left">
            ...
            {customer && (
              <>
                <div className="flex items-start gap-2 text-sm text-text-muted"><MapPin size={15} className="mt-0.5 flex-shrink-0" /> <span className="flex-1">{customer.installation_address}</span></div>
                <div className="flex items-center gap-2 text-sm text-text-muted"><CreditCard size={15} /> Area: {customer.installation_area}</div>
              </>
            )}
          </div>
        </div>
